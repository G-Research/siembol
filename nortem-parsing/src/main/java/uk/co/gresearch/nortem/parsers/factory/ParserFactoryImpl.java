...
import uk.co.gresearch.nortem.parsers.extractors.*;
...
import static uk.co.gresearch.nortem.parsers.model.PreProcessingFunctionDto.STRING_REPLACE;
...
public class ParserFactoryImpl implements ParserFactory {
    ...
    public static ParserFactory createParserFactory() throws Exception {
        JsonSchemaValidator validator = new NortemJsonSchemaValidator(ParsersConfigDto.class);
    ...
    private NortemParser createSyslogParser(SyslogParserConfigDto syslogConfig,
                                            List<ParserExtractor> extractors,
                                            List<Transformation> transformations) {
        NortemSyslogParser.Builder builder = new NortemSyslogParser.Builder()
    ...
    private EnumSet<NortemSyslogParser.Flags> getSyslogFlags(SyslogParserConfigDto syslogConfig) {
        EnumSet<NortemSyslogParser.Flags> ret = EnumSet.noneOf(NortemSyslogParser.Flags.class);
        if (syslogConfig.getMergeSdElements()) {
            ret.add(NortemSyslogParser.Flags.MERGE_SD_ELEMENTS);
        }
        ...
        if (syslogConfig.getSyslogVersion() == SyslogVersionDto.RFC_3164
            ...
            ret.add(NortemSyslogParser.Flags.EXPECT_RFC_3164_VERSION);
        }

        if (syslogConfig.getSyslogVersion() == SyslogVersionDto.RFC_5424
                || syslogConfig.getSyslogVersion() == SyslogVersionDto.RFC_3164_RFC_5424) {
            ret.add(NortemSyslogParser.Flags.EXPECT_RFC_RFC5424_VERSION);
        }
