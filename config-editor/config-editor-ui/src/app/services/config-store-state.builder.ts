...
export class ConfigStoreStateBuilder {
    ...
    computeFiltered(user: string): ConfigStoreStateBuilder {
        ...
        if (this.state.searchTerm !== undefined && this.state.searchTerm !== '') {
            ...
            this.state.filteredDeployment.configs = this.state.filteredDeployment.configs.filter(
                f => f.name.toLowerCase().includes(lowerCaseSearchTerm)
                    || f.author.toLowerCase().startsWith(lowerCaseSearchTerm)
                    || f.tags === undefined ? true : f.tags.join(' ').includes(lowerCaseSearchTerm));

            this.state.filteredConfigs = this.state.filteredConfigs.filter(
                r => r.name.toLowerCase().includes(lowerCaseSearchTerm)
                    || r.author.toLowerCase().startsWith(lowerCaseSearchTerm)
                    || r.tags === undefined ? true : r.tags.join(' ').includes(lowerCaseSearchTerm))
        }
    ...
    removeConfigFromDeployment(filteredIndex: number) {
        this.state.deployment.configs = this.state.deployment.configs
        .filter(x => x.name !== this.state.filteredDeployment.configs[filteredIndex].name);
        return this;
    }

    moveConfigInDeployment(filteredPreviousIndex: number, filteredCurrentIndex: number) {
        const previousIndex = this.state.deployment.configs.findIndex(e =>
            e.name === this.state.filteredDeployment.configs[filteredPreviousIndex].name);
        const currentIndex = this.state.deployment.configs.findIndex(e =>
            e.name === this.state.filteredDeployment.configs[filteredCurrentIndex].name);
