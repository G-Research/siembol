...
import uk.co.gresearch.nortem.enrichments.evaluation.EnrichingRule;
...
public class EnrichmentCompilerImpl implements EnrichmentCompiler {
    ...
    public EnrichmentResult compile(String rules, TestingLogger logger) {
        ...
        if (validationResult.getStatusCode() != NortemResult.StatusCode.OK) {
            return EnrichmentResult.fromNortemResult(validationResult);
        }

        try {
            RulesDto rulesDto = JSON_RULES_READER.readValue(rules);

            List<Pair<String, Rule>> nikitaRules = rulesDto.getRules().stream()
                    .map(this::createNikitaRule)
                    .collect(Collectors.toList());

            EnrichmentEvaluator ruleEvaluator = new NikitaEnrichmentEvaluator.Builder()
                    .rules(nikitaRules)
                    .build();
    ...
    public static EnrichmentCompiler createEnrichmentsCompiler() throws Exception {
        JsonSchemaValidator rulesSchemaValidator = new NortemJsonSchemaValidator(RulesDto.class);
        JsonSchemaValidator testSchemaValidator = new NortemJsonSchemaValidator(TestingSpecificationDto.class);
