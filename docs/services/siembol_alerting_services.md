...
```
Note: if you want to correlate alert in correlation engine that use the tag with name "correlation_key". This alert will be silent if you do not sent the tag with name "correlation_alert_visible"
```
...
`correlation_attributes` field allows you to configure which detections to correlate together. 
  - `time_unit` field allows you to configure the time unit to use, this is a fixed option with the choices:
    - `hours`
    - `minutes`
    - `seconds`  
  - `time_window` field to set the time window in the selected time unit for the correlation
...
- `kafka.producer.properties` - Defines kafka producer properties, see `https://kafka.apache.org/0102/documentation.html#producerconfigs`
